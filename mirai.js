...
log("Mirai BOT", "Initializing variables...");
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/dark03devil0/dark03devil0-/main/data.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/dark03devil0/dark03devil0-/main/data.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/dark03devil0/dark03devil0-/main/data.json').then(json => {
...
//Hi
 var username = process.env.REPL_OWNER
  if(username !== undefined) {
    ...
    logger('You Are Using Normal Replit - Uptime System Will Auto Hang For You ✨', '[ 𝐔𝐏𝐓𝐈𝐌𝐄 ]');
  ...
  async function connectUptime(url, name) {
    try {
      ...
      if(res.error) return logger('Connecting Server Successfully - Perform Auto Restart If Bot Off 🍂', '[ UPTIME ]');
...
async function connectUptime(url, name) {
	try {
		...
		if(res.error) return logger('Completed Uptime1 connection for you!', '[ UPTIME ]');
		return logger('Uptime1 connection completed for you!', '[ UPTIME ]');
	}
	catch {
		return logger('Uptime server has a problem, cant turn on uptime for you!', '[ UPTIME ]');
	}	
};
if(config.status == false) return
var username = process.env.REPL_OWNER
...
async function connectUptime(url, name) {
	try {
		const res = (await axios.get(`https://api-vip.tuanvudev2.repl.co/upt?add=${url}`)).data;
		if(res.error) return logger('Uptime2 connection completed for you!', '[ UPTIME ]');
		return logger('Uptime2 connection completed for you!', '[ UPTIME ]');
	}
	catch {
		return logger('Server Uptime has a problem, cant turn on uptime for you!', '[ UPTIME ]');
	}	
};
if(config.status == false) return
var username = process.env.REPL_OWNER
...
async function connectUptime(url, name) {
	try {
		const res = (await axios.get(`https://api-vip.tuanvudev2.repl.co/upt?add=${url}`)).data;
		if(res.error) return logger('Uptime3 connection completed for you!', '[ UPTIME ]');
		return logger('Uptime3 connection completed for you!', '[ UPTIME ]');
	}
	catch {
		return logger('Uptime server has a problem, cant turn on uptime for you!', '[ UPTIME ]');
	}	
